...
| 基金产品 | 388 只 |
| 策略类型 | 7 种 |
| 覆盖周数 | 28 周（2026/01/09 – 2026/07/24）|
...
解析 `data/` 下的 Excel 周报（正本在 `点睛焱究所/4. 周度业绩排名更新及业绩点评/周度业绩/2026/量化股票/`，当前 29 个、覆盖 0105–0724；导入前需复制进 `data/`），每个文件含 7 个策略 sheet，列布局因周次略有差异（14–17 列）。脚本自动识别列名，统一入库到 SQLite。
...
*数据来源：点睛业绩放送 周度 Excel 报告（2026/01 – 2026/07）*
